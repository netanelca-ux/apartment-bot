...
SEARCH_URL = (
    ...
    "?topArea=2&city=5000"
    f"&rooms={SEARCH_CRITERIA['rooms']}-{SEARCH_CRITERIA['rooms']}"
    ...
    async def capture_json_feed(response):
        ...
        try:
            ...
            if items:
                ...
                logger.debug(f"Yad2: captured {len(items)} items from {url}")
        except Exception as e:
            logger.debug(f"Yad2: could not parse {url}: {e}")
...
async def _parse_dom(page) -> list[dict]:
    """DOM parser — finds all listing links and extracts surrounding card text."""
    try:
        await page.wait_for_selector('a[href*="/realestate/item/"]', timeout=15_000)
    except Exception:
        logger.warning("Yad2 DOM: no listing links found")
        return []

    # Link-first approach: start from the known links, walk up to the card container
    cards = await page.evaluate("""
        () => {
            ...
            const results = [];
            links.forEach(a => {
                const idMatch = a.href.match(/\\/item\\/[^\\/]+\\/([^?#]+)/);
                const id = idMatch ? idMatch[1] : null;
                if (!id || seen.has(id)) return;
                seen.add(id);

                // Walk up to find the card container (stops at feedItemBox or after 6 levels)
                let container = a;
                for (let i = 0; i < 6; i++) {
                    if (!container.parentElement) break;
                    container = container.parentElement;
                    if (container.className && container.className.includes('feedItemBox')) break;
                }
    ...
    for card in cards:
        ...
        # Price format: "₪ 4,800" (₪ comes first)
        ...
        # Neighborhood: in "דירה, NEIGHBORHOOD, תל אביב יפו" pattern
        neighborhood = _extract_neighborhood(text)

        lines = [l.strip() for l in text.split("\n") if l.strip()]
        # Address: first non-price, non-city, non-rooms line after line 0
        address = ""
        for line in lines[1:]:
